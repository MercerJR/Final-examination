...
var WuZiQi = {
    ...
    playchess:function(e){
        if(bout&&color!=""){
            ...
            chess.xy = $(e).attr("id");
            chess.color = color;
            chess.message = "系统：您已落子，请等待对手落子！";
            chess.bout = false;
            if(websocket!=null){
                websocket.send(JSON.stringify(chess));
            }else{
...
//	$(canvas).css({"position":"relative","top":(widthAndHeight/2)+"px","left":(widthAndHeight/2)+"px","z-index":9999});
    ...
    if('WebSocket' in window){
        websocket = new WebSocket("ws://"+window.location.host+"/chess");
    }
    else{
    ...
    websocket.onmessage = function(){
        ...
        if(chess.message!=""){
            $("#messageContent").append(chess.message);
            $("#messageContent").append("\n");
            //将多行文本滚动总是在最下方
            $("#messageContent").scrollTop($("#messageContent")[0].scrollHeight - $("#messageContent").height());
        }
        if(chess.xy!=""&&chess.color!=""){
            $("#"+chess.xy).html("<div class=\"chessman "+chess.color+"\"></div>");
            bout = chess.bout;//落子后才改状态
            WuZiQi.isEnd(chess.xy,chess.color);
        }else if(chess.xy==""&&chess.bout){//没有坐标且bout为true，则为对局首次开始落子
            bout = chess.bout;
        }

        if(chess.xy==""&&chess.color!=""){//没有坐标，但有颜色，则为首次赋予棋子颜色
            color = chess.color;
        }
    };
